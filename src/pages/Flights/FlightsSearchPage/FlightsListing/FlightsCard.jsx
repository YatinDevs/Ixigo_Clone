...
import { airlineData } from "../../../../constants";
    ...
    <article className="card w-full border-2 relative rounded-xl h-fit py-4 px-2 hover:shadow-even transition-all bg-white  duration-500 scale-[98%] hover:scale-100">
      ...
      <div className="flex justify-between items-center gap-4 ">
        ...
        <div className="duration basis-[23.3%] text-center">
          <div
            className={`stop text-xs truncate text-slate-500  capitalize border-b-4 ${
              duration > 2
                ? duration < 5
                  ? "border-yellow-300"
                  : " border-red-300"
      ...
      <div
        className="font-medium text-center text-xs text-blue-600 cursor-pointer select-none transition-all"
        onClick={() => {
      ...
      >
        {showDetails ? (
          <>
            Hide Details <FaAngleUp className="inline" />
          </>
        ) : (
          <>
            Show Details <FaAngleDown className="inline" />
          </>
        )}{" "}
      </div>
    </article>
